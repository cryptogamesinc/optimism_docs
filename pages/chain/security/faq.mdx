---
タイトル OPメインネットのセキュリティ・モデル
言語: ja-JP
説明 OPメインネットのセキュリティ・モデルと、よくある質問に対する回答。
---
...
OPメインネットは現在進行中です。
OPメインネットのユーザーを保護するセキュリティ・メカニズムの絶え間ない反復的な改善は、[Optimism Collective](https://community.optimism.io/docs/governance/)全体の最優先事項です。
Optimism Collectiveは、OPメインネットおよびOPスタック全体のセキュリティについて、明確かつ透明性を保つよう努めています。

## 結論

どのようなブロックチェーン・システムのセキュリティ・モデルも、その最小公倍数と同じ強さしかありません。
...
最初のフォールト・プルーフ・メカニズムのアルファ版は[現在テスト中](https://blog.oplabs.co/building-a-fault-proof-system/)である。

<Callout
フォールト・プルーフは銀の弾丸**ではなく、**フォールト・プルーフがシステムのセキュリティを有意に向上させるわけではないことを理解することが重要です。
OPメインネットは、最終的にインスタント・アップグレードの必要性を完全になくすよう設計されたマルチクライアント・マルチプルーフ・アプローチに従っている。
</Callout
...
**7日間のチャレンジ・ウィンドウ（「高速アップグレード・キー」）***内にアップグレードできるシステムであれば、フォールト・プルーフを導入しても、システムのセキュリティは実質的に向上しません。
OPメインネットのような高速アップグレード鍵を持つシステムは、セキュリティの面でアップグレード鍵に完全に依存している。
OPメインネットの目標は、高速アップグレード・キーがなくても、それだけでシステムを保護できるフォールト・プルーフを導入した最初のシステムになることです。
